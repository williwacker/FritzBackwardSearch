...
class FritzCalls(object):
    ...
    def remove_not_found(self, numberlist_in):
        ...
        try:
            ...
            nameNotFoundList = open(self.notfoundfile, 'w+').readlines()
    ...
    def get_names(self, searchlist):
        ...
        for number in searchlist.keys():
            ...
            if number.startswith("00"):
                fullNumber = ""
                writeLog("Ignoring international number {}".format(number))
...
#					name = self.dasoertliche(searchlist[number])
    ...
    def dasoertliche(self, number):
        ...
        if name:
            writeLog('{} = dasoertliche({})'.format(number, name.get_text().encode(
...
class FritzPhonebook(object):
    ...
    def __init__(self, connection, name):
        ...
        if self.bookNumber == -1:
            writeLog('Phonebook: {} not found !'.format(name), True)
            exit(1)
        self.get_phonebook()
...
class FritzBackwardSearch(object):
    ...
    def runSearch(self, s=''):
        ...
        if args.searchnumber:
            ...
            writeLog("Searching for {}".format(searchnumber))
        if s:
...
#		else:
#			writeLog("Searchnumber nicht gesetzt")
        unknownCallers = calls.remove_not_found(unknownCallers)
#		writeLog("Length unknownCallers = {}".format(len(unknownCallers)))
        knownCallers = calls.get_names(unknownCallers)
        phonebook.add_entry_list(knownCallers)

    def influxClient(self, host='raspberrypi2', port=8086):
        influx_client = InfluxDBClient(
            host, port, self.prefs['influx_user'],
            self.prefs['influx_password'],
            self.prefs['influx_database'])
    #	influx_client.drop_database(self.influx_database)
    #	influx_client.create_database(self.influx_database)
        return influx_client


def writeInfluxDB(influx_client, origin, value):
    json_body = [
        {
            "measurement": "fritzboxLog",
            "tags": {
                "Origin": origin,
            },
            "fields": {
...
def writeLog(logString, print_to_console=False):
    ...
    if print_to_console:
        print(logString)
#	writeInfluxDB(influx_client2,os.path.basename(sys.argv[0])[::-1][3:][::-1],logString)
#	print(influx_client.query('select * from fritzboxLog;'))


if __name__ == '__main__':
    FBS = FritzBackwardSearch()
#	influx_client2 = FBS.influxClient()
